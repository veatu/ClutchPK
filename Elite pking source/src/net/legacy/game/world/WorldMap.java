...
public class WorldMap {
    ...
    private static boolean prepareWorldMap(String fileName) {
        ...
        if (!dbFile.exists() || fileName.indexOf(".xml") < 0) {
            Miscellaneous.println("[ERROR]: Unable to load world map: " + fileName);
            return false;
        }
        try {
    ...
    private static void loadWorldMap(Document worldMap) {
        if (worldMap == null) {
            Miscellaneous.println("[ERROR]: world map not found.");
            return;
        }
        ...
        if (nl != null && nl.getLength() > 0) {
            for (int i = 0; i < nl.getLength(); i++) {
                try {
                    ...
                    Miscellaneous.println("[ERROR]: loading worldobject(" + i + ") failed.");
                    _ex.printStackTrace();
                    continue;
                }
    ...
    public static boolean isWalkAble(int height, int absX, int absY, int toAbsX, int toAbsY) {
        ...
        if (direction1 == -1 || direction2 == -1) {
            Miscellaneous.println("Direction Bug from(" + absX + "," + absY + ") to (" + toAbsX + "," + toAbsY + ") Height(" + height + ") abs(" + toAbsX + "," + toAbsY + ")");
            return false;
        }
        for (int i = 0; i < maxObjects; i++) {
